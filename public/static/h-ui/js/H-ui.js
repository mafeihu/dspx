...
function displaynavbar(obj){
	if($(obj).hasClass("open")){
		...
		$("#page-content").addClass("ml170");
	}else{
		$(obj).addClass("open");
		$("body").addClass("big-page");
		$("#page-content").removeClass("ml170");
	...
	$(obj).on(Event,function(){
		if($(this).next().is(":visible")){
			...
			if(obj_type == 3){
				...
				if($(this).find("b")){
					$(this).find("b").html("-");
					menu_key = $(this).attr('menu_key');
					if(menu_key){
						$.cookie('menu_key',null,{ path: "/"});
						$.cookie('menu_key', menu_key, { path: "/"});
					}
				}
			}
		}
...
$(function(){
	...
	$(document).on("click",".Hui-tags-token",function(){
		...
		var menu_key = $.cookie('menu_key',{ path: "/"});
		if(menu_key){
			console.log('._menu'+menu_key);
			$('._menu'+menu_key).show();
		}
});
